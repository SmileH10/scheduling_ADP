...
class QNet(object):
    ...
    def initialize_model(self):
        ...
        while hop < self.T:  # mu[0][n]: 초기값. mu[1][n] ~ mu[T][n] 까지 새로 구함
            ...
            for n1 in self.g.n_x.keys():
                ...
                mu[hop][n1] = tf.keras.activations.relu(input_mu)
    ...
    def run_job_selection(self, mc, simenv_now):
        ...
        if np.random.random() < self.epsilon:
            ...
            for a in action_cands_wait:
                ...
                qhat = self.model.predict(np.expand_dims(x, axis=0))
                if qhat < best_q:
                    best_q = qhat
                    best_a = a
    ...
    @staticmethod
    def make_input_shape(n_x):  # qnet으로 옮기기
        x = []
        for n in n_x.keys():
            if n == 'S':
                x.extend([1, 0])
                ...
                for att in n_x[n].keys():
                    x.append(n_x[n][att])
        return x

    def save_state_action(self, post_n_x, t):
        num_waitjob = sum(self.post_n_x[n]['wait'] for n in self.post_n_x.keys())
        if self.short_memory == 0:
    ...
    def train_model(self):
        ...
        self.model.fit(y, q, batch_size=BATCH_SIZE)
