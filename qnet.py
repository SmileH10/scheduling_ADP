...
class QNet(object):
    def __init__(self, sim_env, mc_name_list, mc_sqc, proc_t, num_ptrn):
        ...
        self.time = {'get_state': 0.0, 'get_action_list': 0.0, 'get_qvalue': 0.0, 'job_select': 0.0, 'job_queue_control': 0.0,
                     'save_state_action': 0.0}
    ...
    def build_basic_state(self, toy=True):
        ...
        for ptrn in range(24):
            for oper in range(len(self.mc_sqc[ptrn])):  # 'A' in  ['A', 'B', 'C']
                if oper != 'END':
                    conj_edges_af.append(((ptrn, oper), (ptrn, oper + 1)))
                if oper != 'START':
                    conj_edges_bf.append(((ptrn, oper - 1), (ptrn, oper)))
    ...
    def initialize_model(self, toy=True):
        ...
        if toy:
            ...
            input = Add()([x[n] for n in self.nodes])
            output = Dense(32, activation='relu')(input)
            qhat = Dense(1)(output)
            self.model = Model(inputs=[x[n] for n in self.nodes],
                               outputs=qhat)
    ...
    def get_state(self, toy=True):
        ...
        if toy:
            ...
            for ptrn in range(self.num_ptrn):
                ...
                node_att[(ptrn, len(self.mc_sqc[ptrn]) - 1)] = [0, 0]
            state = [node_att[self.nodes[i]] for i in range(len(self.nodes))]
            # print("QNet::get_state Ends")
            self.time['get_state'] += time() - start
            return state
        else:
            node_att = {}
            for ptrn in range(self.num_ptrn):
    ...
    def get_action_list(self, state=False, mc_name=0):
        ...
        if mc_name:
            ...
            for i in range(len(self.nodes)):
                if self.nodes[i][1] < len(self.mc_sqc[self.nodes[i][0]]) - 1:
            ...
            print('mc_numque: ', mc_numque)
            print('mc_numuser: ', mc_numuser)
            for mc in self.mc_name_list:
            ...
            for i in range(len(self.nodes)):
                ...
                if oper_num < len(self.mc_sqc[ptrn]) - 1 and self.mc_sqc[ptrn][oper_num] == mc_name:
                    ptrn = self.nodes[i][0]
                    oper_num = self.nodes[i][1]
        ...
        self.time['get_action_list'] += time() - start
        return action_list
    ...
    def get_qvalue(self, state, action_node):
        ...
        # print(np.array([2, 1]).shape, np.expand_dims(np.array([2,1]), axis=0).shape)
        qhat = self.model.predict([np.expand_dims(state_action[i], axis=0) for i in range(len(state_action))])
        self.time['get_qvalue'] += time() - start
        return qhat
